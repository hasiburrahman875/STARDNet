...
def generate_half_video_split():
    splits = ['train', 'val']
    videos_list = glob.glob('//cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/Videos/*.avi')
    dest_dir = '/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl_drones'
    annotation_dir = '/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/Annotations'
    frames_dir = '/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/frames' #zero index frames
    ...
    for video_path in tqdm(videos_list):
        ...
        for split in splits:
            ...
            for fiddest, fid in enumerate(iterator):
                ...
                assert os.path.exists(src_image_path), f"Fatal error {src_image_path} doesn't exists"
...
def generate_visdrone_annotations():
    ...
    train_ids = [1,11,12,18,19,29,37,46,47,48,49,53,55,56]
    val_ids = [1,11,12,18,19,29,37,46,47,48,49,53,55,56]
    ...
    dest_dir = '/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl_drones'
    annotation_dir = '/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/Annotations'
    frames_dir: str = '/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/frames' #zero index frames
    for split in splits:
        dest_dir_images = os.path.join(dest_dir, f"{split}", "frames")
        dest_dir_annotations = os.path.join(dest_dir, f"{split}", "labels")
        dest_dir_videos = os.path.join(dest_dir, f"{split}", "videos")
        ...
        for video_path in videos_list:
            ...
            for fid in tqdm(range(num_frames), desc=f"Copying Video frames for.. {video_name}", total=num_frames):
                ...
                src_image_path = os.path.join(frames_dir, f"{video_name}_{str(fid).zfill(5)}.png")
                assert os.path.exists(src_image_path), print(f"Fatal error {src_image_path} doesn't exists")
                #os.symlink(src_image_path, dest_image_path)
...
def renaming_operation(fl_drones_yolo_data_root):#change fl-drones naming to NPS naming
    ...
    for split in splits:
        ...
        annotations_dir = os.path.join(fl_drones_yolo_data_root, f"{split}", "labels1")
        frames_paths = glob.glob(os.path.join(frames_dir, "*"))
        labels_paths = glob.glob(annotations_dir+"/*.txt")
...
def copy_current_annotations_to_annotations_mains():
    ...
    '''
    splits = ["train", "val"]
    annotations_root_dir = "/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/Annotations"
    video_root_dir = "/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/Videos"
    for split in splits:
        current_annotations_dir = f"/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/Annotations/{split}/labels"
        current_video_dir = f"/cluster/pixstor/madrias-lab/Hasibur/STARDNet/Datasets/fl/Videos{split}"
...
if __name__ == "__main__":
    #split ='val'
    #renaming_operation(f"/home/mrpk9/Videos/fl_drones")
