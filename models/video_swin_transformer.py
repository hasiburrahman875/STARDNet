...
class _DW3DUnitSO(nn.Module):
    def __init__(self, c:int, tks:int=3, sks:int=3, dil:int=1, use_se:bool=True):
        super().__init__()
        pad_t = tks // 2
        pad_s = (sks + (sks - 1) * (dil - 1)) // 2
        self.dw_t = nn.Conv3d(c, c, (tks,1,1), padding=(pad_t,0,0), groups=c, bias=False)
        self.bn_t = nn.BatchNorm3d(c)
        self.dw_s = nn.Conv3d(c, c, (1,sks,sks), padding=(0,pad_s,pad_s), dilation=(1,dil,dil), groups=c, bias=False)
        self.bn_s = nn.BatchNorm3d(c)
        self.pw   = nn.Conv3d(c, c, 1, bias=False)
        self.bn_pw= nn.BatchNorm3d(c)
        self.act  = nn.SiLU(inplace=False)              # <- no in-place
        self.use_se = use_se
        if use_se:
            r = max(1, c//32)
            self.se1 = nn.Conv3d(c, r, 1, bias=True)
            self.se2 = nn.Conv3d(r, c, 1, bias=True)

    def forward(self, x):
        r = x
        x = self.act(self.bn_t(self.dw_t(x)))
        x = self.act(self.bn_s(self.dw_s(x)))
        x = self.act(self.bn_pw(self.pw(x)))
        if self.use_se:
            w = F.adaptive_avg_pool3d(x, 1)
            w = F.silu(self.se1(w))                     # not in-place
            w = torch.sigmoid(self.se2(w))
            x = x * w                                   # not in-place
        return x + r


class LiteTemporalConv3D_SmallObject(nn.Module):
    """
    Signature compatible with SwinTransformerBlock3D:
        LiteTemporalConv3D_SmallObject(c1, num_frames=5, num_layers=2, **kwargs)

    It stacks num_layers copies of a very light depthwise 3D unit.
    """
    __constants__ = ['num_frames']
    ...
    def __init__(self,
                 ...
                 dilation: int = 1,
                 use_se: bool = True,
                 **kwargs):  # absorb any extra kwargs the builder might pass
        super().__init__()
        self.num_frames = int(num_frames)
        depth = max(1, int(num_layers))
        self.blocks = nn.Sequential(*[
            _DW3DUnitSO(c1, tks=t_kernel, sks=s_kernel, dil=dilation, use_se=use_se)
            for _ in range(depth)
        ])

    def _to_5d(self, x):
        # x: [B,C,H,W] with B = B' * T
        b, c, h, w = x.shape
        assert b % self.num_frames == 0, f"Batch {b} not divisible by T {self.num_frames}"
        b2 = b // self.num_frames
        return x.reshape(b2, self.num_frames, c, h, w).permute(0, 2, 1, 3, 4).contiguous()  # [B',C,T,H,W]

    def _to_4d(self, x5):
        # x5: [B',C,T,H,W] back to [B'*T,C,H,W]
        b2, c, t, h, w = x5.shape
        return x5.permute(0, 2, 1, 3, 4).reshape(b2 * t, c, h, w).contiguous()
...
# --- Drop-in lightweight replacement ---
class mergeDet(nn.Module):
    def __init__(self, c1, num_frames: int = 5, num_layers: int = 2):
        super().__init__()
        self.core = LiteTemporalConv3D_SmallObject(
            c1=c1,
            num_frames=num_frames,
            num_layers=num_layers,  # keep same knob as original
            t_kernel=3,
            s_kernel=3,
            dilation=1,
            use_se=True,
    ...
    def forward(self, x):  # x: [B, C, H, W] with B = batch_size * num_frames
        return self.core(x)
